...
- **Quarterly** (up to 12 set + 10 drawn): the quarter's three biggest highlights (each month's highlight and your 🟢 wins listed in the question) and three biggest lowlights (your 🔴 nags and the months' moods), *did you achieve your quarterly goals?*, then the OKR checkpoint one tier up: 🥅 objective by objective (hit, partial or miss, the deciding factor, and whether the bar was too high or too low), 🎉 the year's goal with how many quarters it has left (ahead, on track or behind, what next quarter must deliver, and whether the review alters the yearly goal; in Q4 which goals carry into next year), ⏪ this quarter against last (completed, focus, mood and income side by side, and the wish you wrote last quarter quoted back), 🔄 habit consistency for the quarter, 💰 income per month with its chip (does it align with your forecast, what to improve, an expense category to cut), ⏱ what effort is not worth your time with the quarter's focus hours listed, when you felt most passionate and when bored, your top three priorities, and where you want to be in three months at your current pace. "What is on your mind?" closes the set block; ten reflection prompts follow, two each from lookback, lessons, decisions, direction and system, cycling so each prompt comes once in every three quarters and never two quarters running. Then the goal editor opens aimed at the **next** quarter, and stays open until you Esc. Lines that read the note only appear while the note has the section to read.
...
- Then ✨ Highlight, 📊 Stats (with **Monthly Completed** bars and focus by month), 💿 Data (moods by month, income by month, the quarter's 🎂 dates), **⏪ Last quarter** (its headline numbers, plus `🔮 Wanted:` - where last quarter's journal said it wanted to be by now, so this quarter's journal can ask whether you got there), **📔 Quarterly journal** and **♻️ Quarterly Review**.
