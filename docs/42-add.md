...
- **Subtasks**: `|` separates them. `Buy groceries | Milk | Bread ~Money` creates the task with two subtasks in the order you typed. The pipes only carve the title, so every other token still applies to the whole add. A `➕ Another subtask` row sits under the preview and hands the line back with one more `|`, while the `Create:` row above stays the way out. When the add is already aimed into a parent (the ⌘ Actions `➕ Add task` row on a TASK, or `~p`), every segment becomes a subtask of that parent instead. Aimed at a **list** (⌘ Actions `➕ Add task` on a list row) there is no parent to hang from, so every segment becomes its own task in that list and the row reads `➕ Another task` - the same keep-the-line-open flow, one level up. A `|` inside a `[[link]]` is text, not a separator.
