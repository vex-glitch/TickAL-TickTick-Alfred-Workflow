...
**Keyword:** `tur` · **Hotkey:** (set in canvas) - saves the active browser tab as a task. Notes, links, and images otherwise live inside the `tse`, `tad`, and ⌘⏎ flows below.
...
In a CRM add, the `[[` picker scopes to bookings - see [CRM](47-crm.md).
...
Both need PyObjC (it reads the clipboard - `tup` → **Install PyObjC**, see [Setup](30-setup.md#focus-bar)) and the one-time v2 session token - see [Setup](30-setup.md). Without the token, the attach reports a message pointing at the Settings rows that store one. CRM booking adds attach the clipboard image automatically - see [CRM](47-crm.md).
...
- [Focus](46-focus.md) - sticky-paired sessions, staging blocks
- [CRM](47-crm.md) - booking image auto-attach, scoped `[[` picker
