...
# HEAL BEFORE PLANNING: TickAL stored 29 Sep - 14 Oct, wanted 19 Sep - 14 Oct.
# Putting it on 19 Sep - 14 Oct is the span it already HAS: a plan on the
# stale span read that as a -10 start delta and pulled every KR in ten days.
moves, heals = okr.ripple_plan(LI, "tal", d(9, 19), d(10, 14))
check("TickAL on its own wanted span is a no-op: only itself, on that span",
      moves == [("tal", d(9, 19), d(10, 14))], str(moves))
check("...and nothing left to heal", heals == [], str(heals))
...
moves, heals = okr.ripple_plan(LI, "k_rev", d(9, 26), d(9, 28))
check("a plan elsewhere still lists TickAL's stale span in its heals (judged vs STORED)",
      moves == [("k_rev", d(9, 26), d(9, 28))] and heals == [("tal", d(9, 19), d(10, 14))],
      str((moves, heals)))
moves, heals = okr.ripple_plan(LI, "tal", d(9, 20), d(10, 15))
check("a real TickAL move reads its deltas off the healed span (+1, not -9)",
      by_id(moves).get("k_goal") == (d(9, 20), d(9, 22))
      and by_id(moves).get("k_pub") == (d(10, 13), d(10, 15)), str(moves))
...
RIP = [
    ...
    D("oa", "🥅 O • A", d(10, 1), d(10, 9), parent="y1", kids=["a1", "a2", "a3", "apar"]),
    D("a1", "🔑 KR • a1", d(10, 1), d(10, 3), parent="oa"),
    D("a2", "🔑 KR • a2", d(10, 4), d(10, 6), parent="oa"),
    D("a3", "🔑 KR • a3", d(10, 7), d(10, 9), parent="oa"),
    D("apar", "🔑 KR • parallel", d(10, 2), d(10, 8), parent="oa"),
    D("ob", "🥅 O • B", d(10, 10), d(10, 15), parent="y1", kids=["b1", "b2", "bu"]),
    D("b1", "🔑 KR • b1", d(10, 10), d(10, 12), parent="ob"),
    D("b2", "🔑 KR • b2", d(10, 13), d(10, 15), parent="ob"),
    D("bu", "🔑 KR • undated", parent="ob"),
    D("y2", "🏔️ Y • Lane two", d(10, 5), d(10, 7), kids=["ox"]),
    D("ox", "🥅 O • X", d(10, 5), d(10, 7), parent="y2", kids=["x1"]),
    D("x1", "🔑 KR • x1", d(10, 5), d(10, 7), parent="ox"),
    D("ot", "🥅 O • No Y", d(10, 1), d(10, 6), kids=["t1", "t2"]),
    D("t1", "🔑 KR • t1", d(10, 1), d(10, 3), parent="ot"),
    D("t2", "🔑 KR • t2", d(10, 4), d(10, 6), parent="ot"),
]
RI = okr.items_from(RIP)
check("the ripple fixture starts healed", okr.heal_diff(RI) == [], str(okr.heal_diff(RI)))

moves, heals = okr.ripple_plan(RI, "a1", d(10, 1), d(10, 5))
mv = by_id(moves)
check("extend: the moved item first", moves[0] == ("a1", d(10, 1), d(10, 5)))
check("extend: successors in the Y shift by D=2",
      mv.get("a2") == (d(10, 6), d(10, 8)) and mv.get("a3") == (d(10, 9), d(10, 11))
      and mv.get("b1") == (d(10, 12), d(10, 14)) and mv.get("b2") == (d(10, 15), d(10, 17)),
      str(mv))
check("extend: a later O in the lane shifts too", mv.get("ob") == (d(10, 12), d(10, 17)))
check("extend: the parallel item stays", "apar" not in mv)
check("extend: undated stays", "bu" not in mv)
check("extend: the other Y is untouched", not {"y2", "ox", "x1"} & set(mv))
check("extend: the no-Y lane is untouched", not {"ot", "t1", "t2"} & set(mv))
check("extend: ancestors never shift", not {"oa", "y1"} & set(mv))
check("extend: ancestors heal", by_id(heals) == {"oa": (d(10, 1), d(10, 11)),
                                                  "y1": (d(10, 1), d(10, 17))}, str(heals))

moves, heals = okr.ripple_plan(RI, "a2", d(10, 7), d(10, 9))
mv = by_id(moves)
check("move later: +3 from after a2's old end",
      mv == {"a2": (d(10, 7), d(10, 9)), "a3": (d(10, 10), d(10, 12)),
             "ob": (d(10, 13), d(10, 18)), "b1": (d(10, 13), d(10, 15)),
             "b2": (d(10, 16), d(10, 18))}, str(mv))
check("move later: the item before it and the parallel one stay",
      "a1" not in mv and "apar" not in mv)

moves, heals = okr.ripple_plan(RI, "a2", d(10, 3), d(10, 5))
mv = by_id(moves)
check("move earlier: -1, symmetric",
      mv == {"a2": (d(10, 3), d(10, 5)), "a3": (d(10, 6), d(10, 8)),
             "ob": (d(10, 9), d(10, 14)), "b1": (d(10, 9), d(10, 11)),
             "b2": (d(10, 12), d(10, 14))}, str(mv))
check("move earlier: parents heal to the new shape",
      by_id(heals).get("y1") == (d(10, 1), d(10, 14)), str(heals))

moves, heals = okr.ripple_plan(RI, "oa", d(10, 6), d(10, 14))
mv = by_id(moves)
check("an O moved: its KRs move with it (+5), the parallel one too",
      mv.get("a1") == (d(10, 6), d(10, 8)) and mv.get("a3") == (d(10, 12), d(10, 14))
      and mv.get("apar") == (d(10, 7), d(10, 13)), str(mv))
check("an O moved: the next O in its Y shifts by D",
      mv.get("ob") == (d(10, 15), d(10, 20)) and mv.get("b2") == (d(10, 18), d(10, 20)))
check("an O moved: its undated KR stays undated", "bu" not in mv)
check("an O moved: its Y heals", by_id(heals) == {"y1": (d(10, 6), d(10, 20))}, str(heals))

moves, heals = okr.ripple_plan(RI, "t1", d(10, 2), d(10, 4))
mv = by_id(moves)
check("no Y: the lane is the top-level O",
      mv == {"t1": (d(10, 2), d(10, 4)), "t2": (d(10, 5), d(10, 7))}, str(mv))
check("no Y: the O heals", by_id(heals) == {"ot": (d(10, 2), d(10, 7))}, str(heals))

moves, heals = okr.ripple_plan(RI, "bu", d(10, 20), d(10, 21))
check("an undated item getting dates ripples nothing", moves == [("bu", d(10, 20), d(10, 21))])
check("...but its parents heal around it",
      by_id(heals) == {"ob": (d(10, 10), d(10, 21)), "y1": (d(10, 1), d(10, 21))}, str(heals))
moves, _h = okr.ripple_plan(RI, "a3", d(10, 7), d(10, 9))
check("no change = only itself", moves == [("a3", d(10, 7), d(10, 9))])
check("ripple is pure: the input is untouched", okr.index(RI)["a2"].start == d(10, 4))
for bad, exc in (((RI, "nope", d(10, 1), d(10, 2)), KeyError),
                 ((RI, "a1", d(10, 5), d(10, 1)), ValueError)):
    try:
        okr.ripple_plan(*bad)
        check(f"ripple refuses {exc.__name__}", False)
    except exc:
        check(f"ripple refuses {exc.__name__}", True)
check("lane_of: the Y", okr.lane_of(okr.index(RI)["b1"], RI).id == "y1")
check("lane_of: the top O when there is no Y", okr.lane_of(okr.index(RI)["t2"], RI).id == "ot")

# The successor threshold, exactly: AFTER the moved item's end, not on it.
RI_ON = okr.items_from(RIP + [D("aon", "🔑 KR • starts on a1's last day", d(10, 3), d(10, 3),
                                parent="oa")])
moves, _h = okr.ripple_plan(RI_ON, "a1", d(10, 1), d(10, 5))
mv = by_id(moves)
check("a successor starting ON the moved item's end day stays put", "aon" not in mv, str(mv))
check("...and the one starting the day after moves", mv.get("a2") == (d(10, 6), d(10, 8)))

# A move where the start and the end change by DIFFERENT amounts: the
# subtree follows the start (+2), the lane follows the end (+3).
moves, heals = okr.ripple_plan(RI, "oa", d(10, 3), d(10, 12))
mv = by_id(moves)
check("O move, start +2 end +3: its KRs move by the START delta",
      mv.get("a1") == (d(10, 3), d(10, 5)) and mv.get("a3") == (d(10, 9), d(10, 11))
      and mv.get("apar") == (d(10, 4), d(10, 10)), str(mv))
check("O move, start +2 end +3: the next O in its Y by the END delta",
      mv.get("ob") == (d(10, 13), d(10, 18)) and mv.get("b1") == (d(10, 13), d(10, 15))
      and mv.get("b2") == (d(10, 16), d(10, 18)), str(mv))
check("O move, start +2 end +3: the O heals to its KRs, its Y around both",
      by_id(heals) == {"oa": (d(10, 3), d(10, 11)), "y1": (d(10, 3), d(10, 18))}, str(heals))

# EXTEND ON A PARENT: an end-only change on an O goes to its last KR.
moves, heals = okr.ripple_plan(RI, "oa", d(10, 1), d(10, 11))
mv = by_id(moves)
check("extend an O inside a Y: its last-ending KR takes the +2, and is first",
      moves[0] == ("a3", d(10, 7), d(10, 11)), str(moves))
check("...its other KRs, the parallel one too, stay",
      not {"a1", "a2", "apar", "oa"} & set(mv), str(mv))
check("...the lane after the KR ripples by the same +2",
      mv.get("ob") == (d(10, 12), d(10, 17)) and mv.get("b1") == (d(10, 12), d(10, 14))
      and mv.get("b2") == (d(10, 15), d(10, 17)), str(mv))
check("...and the O and its Y heal to their new ends",
      by_id(heals) == {"oa": (d(10, 1), d(10, 11)), "y1": (d(10, 1), d(10, 17))}, str(heals))
moves, heals = okr.ripple_plan(RI, "y1", d(10, 1), d(10, 18))
check("extend a Y: down through its last O to that O's last KR (b2 +3)",
      moves == [("b2", d(10, 13), d(10, 18))], str(moves))
check("...the O and the Y heal", by_id(heals) == {"ob": (d(10, 10), d(10, 18)),
                                                   "y1": (d(10, 1), d(10, 18))}, str(heals))
moves, heals = okr.ripple_plan(RI, "oa", d(10, 1), d(10, 8))
check("pulling an O's end in is the same road, mirrored (a3 -1, the lane -1)",
      moves[0] == ("a3", d(10, 7), d(10, 8)) and by_id(moves).get("ob") == (d(10, 9), d(10, 14)),
      str(moves))
moves, heals = okr.ripple_plan(RI, "y1", d(10, 3), d(10, 17))
mv = by_id(moves)
check("a Y MOVED (its start changes): the whole subtree shifts by the start delta",
      mv.get("oa") == (d(10, 3), d(10, 11)) and mv.get("a1") == (d(10, 3), d(10, 5))
      and mv.get("b2") == (d(10, 15), d(10, 17)) and "bu" not in mv, str(mv))
RI_ND = okr.items_from([D("yn", "🏔️ Y • Lane", kids=["on", "on2"]),
                        D("on", "🥅 O • Only its own dates", d(10, 1), d(10, 3), parent="yn"),
                        D("on2", "🥅 O • Next", d(10, 4), d(10, 5), parent="yn")])
moves, _h = okr.ripple_plan(RI_ND, "on", d(10, 1), d(10, 5))
check("an O with NO dated deliverable extends itself (nothing to hand it to)",
      moves[0] == ("on", d(10, 1), d(10, 5)) and by_id(moves).get("on2") == (d(10, 6), d(10, 7)),
      str(moves))

# HISTORY NEVER MOVES: done and won't-do stay where they closed.
RIH = okr.items_from([dict(t, status=2) if t["id"] in ("b1", "a2") else
                      dict(t, status=-1) if t["id"] == "b2" else t for t in RIP])
moves, _h = okr.ripple_plan(RIH, "a1", d(10, 1), d(10, 5))
mv = by_id(moves)
# (ob plans on its HEALED span: the won't-do b2 no longer shapes it, 10-12 Oct)
check("history: a done successor and a won't-do one stay; the open ones shift",
      "a2" not in mv and "b1" not in mv and "b2" not in mv
      and mv.get("a3") == (d(10, 9), d(10, 11)) and mv.get("ob") == (d(10, 12), d(10, 14)), str(mv))
moves, _h = okr.ripple_plan(RIH, "oa", d(10, 6), d(10, 14))
mv = by_id(moves)
check("history: a parent's move leaves its done KR behind",
      "a2" not in mv and mv.get("a1") == (d(10, 6), d(10, 8)), str(mv))
RIH2 = okr.items_from([dict(t, status=2) if t["id"] == "a3" else t for t in RIP])
moves, heals = okr.ripple_plan(RIH2, "oa", d(10, 1), d(10, 11))
check("history: extending an O passes over a done last KR to the last OPEN one,"
      " which takes the O's NEW END (not just +D)",
      moves[0] == ("apar", d(10, 2), d(10, 11)), str(moves))
check("...so the O lands on the requested end and no hole opens before O B",
      by_id(heals).get("oa") == (d(10, 1), d(10, 11))
      and by_id(moves).get("ob") == (d(10, 12), d(10, 17)), str((moves, heals)))

# The lane follows the PARENT's old end: an O running inside O A's tail is
# inside it, not after it (review 2026-09-18 - it used to shift).
RTAIL = okr.items_from([
    D("yt", "🏔️ Y • Tail", d(10, 1), d(10, 15), kids=["ta", "tc", "tb"]),
    D("ta", "🥅 O • A", d(10, 1), d(10, 10), parent="yt", kids=["t1", "t2"]),
    D("t1", "🔑 KR • open", d(10, 1), d(10, 5), parent="ta"),
    D("t2", "🔑 KR • done", d(10, 6), d(10, 10), parent="ta", status=2),
    D("tc", "🥅 O • C inside A's tail", d(10, 8), d(10, 9), parent="yt"),
    D("tb", "🥅 O • B", d(10, 11), d(10, 15), parent="yt")])
moves, heals = okr.ripple_plan(RTAIL, "ta", d(10, 1), d(10, 12))
mv = by_id(moves)
check("extend past a done last KR: the open KR takes the new end",
      moves[0] == ("t1", d(10, 1), d(10, 12)), str(moves))
check("...the O inside A's tail stays, the O after A shifts +2",
      "tc" not in mv and mv.get("tb") == (d(10, 13), d(10, 17)), str(mv))
check("...A heals onto the requested end", by_id(heals).get("ta") == (d(10, 1), d(10, 12)),
      str(heals))

# A child O dated by hand with no dated KR yet shapes its Y like a KR does,
# so extending the Y lands on it (review 2026-09-18: it used to move a KR).
RLEAF = okr.items_from([
    D("yl", "🏔️ Y • Leafless", d(10, 1), d(10, 20), kids=["la", "ln"]),
    D("la", "🥅 O • A", d(10, 1), d(10, 12), parent="yl", kids=["l1"]),
    D("l1", "🔑 KR • a1", d(10, 1), d(10, 12), parent="la"),
    D("ln", "🥅 O • New, no KRs yet", d(10, 10), d(10, 20), parent="yl")])
check("the leafless fixture starts healed", okr.heal_diff(RLEAF) == [])
moves, heals = okr.ripple_plan(RLEAF, "yl", d(10, 1), d(10, 23))
check("extend a Y whose end is a KR-less O: that O takes the new end",
      moves == [("ln", d(10, 10), d(10, 23))], str(moves))
check("...and the Y heals onto it", by_id(heals) == {"yl": (d(10, 1), d(10, 23))}, str(heals))

# Pulling a parent's end in when ANOTHER open KR still ends after it cannot
# land: refused, naming the KR to move - never a plan the heal undoes.
RPULL = okr.items_from([
    D("op", "🥅 O • Pull", d(10, 1), d(10, 10), kids=["p1", "p2"]),
    D("p1", "🔑 KR • long", d(10, 1), d(10, 10), parent="op"),
    D("p2", "🔑 KR • tail", d(10, 6), d(10, 10), parent="op")])
try:
    okr.ripple_plan(RPULL, "op", d(10, 1), d(10, 8))
    check("a pull-in the O cannot land on is refused", False)
except ValueError as e:
    check("a pull-in the O cannot land on is refused, naming the KR",
          "cannot end on" in str(e) and ("long" in str(e) or "tail" in str(e)), str(e))
try:
    okr.ripple_plan(RI, "oa", d(10, 1), d(10, 4))
    check("a pull-in past the last KR's start is refused", False)
except ValueError as e:
    check("a pull-in past the last KR's start is refused", "bad span" in str(e), str(e))

# A closed item is history: a schedule action on it is refused.
try:
    okr.ripple_plan(RIH, "a2", d(10, 20), d(10, 22))
    check("a done item cannot be scheduled", False)
except ValueError as e:
    check("a done item cannot be scheduled", "closed" in str(e), str(e))

# Ancestors never shift, even when they start after the moved item: a KR's
# dated subtask lying BEFORE its KR moves alone.
RANC = okr.items_from([
    D("oq", "🥅 O • Anc", d(10, 5), d(10, 9), kids=["kq"]),
    D("kq", "🔑 KR • the KR", d(10, 5), d(10, 9), parent="oq", kids=["sq"]),
    D("sq", "a dated subtask", d(10, 1), d(10, 2), parent="kq")])
moves, _h = okr.ripple_plan(RANC, "sq", d(10, 1), d(10, 3))
check("a subtask before its KR moves alone - its KR and O are ancestors",
      moves == [("sq", d(10, 1), d(10, 3))], str(moves))

# schedule_plan reads the dates off the HEALED span: "extend TickAL +2" off
# its stale stored span (29 Sep) used to become a subtree move.
moves, heals = okr.schedule_plan(LI, "tal", "extend", 2, today=d(9, 18))
check("schedule extend TickAL +2: Publish takes the new end, Goals wf stays",
      moves[0] == ("k_pub", d(10, 12), d(10, 16)) and "k_goal" not in by_id(moves), str(moves))
check("...and TickAL heals onto 19 Sep - 16 Oct",
      by_id(heals).get("tal") == (d(9, 19), d(10, 16)), str(heals))
moves, _h = okr.schedule_plan(RI, "a2", "tomorrow", today=d(10, 1))
check("schedule tomorrow keeps the length", moves[0] == ("a2", d(10, 2), d(10, 4)), str(moves))
moves, _h = okr.schedule_plan(RI, "bu", "tomorrow", today=d(10, 1))
check("schedule tomorrow on an undated item = one day", moves == [("bu", d(10, 2), d(10, 2))],
      str(moves))
moves, _h = okr.schedule_plan(RI, "a2", "date", d(10, 20))
check("schedule pick a date keeps the length", moves[0] == ("a2", d(10, 20), d(10, 22)), str(moves))
for args, why in (((RI, "bu", "extend", 2), "no dates"), ((RI, "a2", "date", None), "no date"),
                  ((RI, "a2", "later"), "unknown")):
    try:
        okr.schedule_plan(*args)
        check(f"schedule refuses: {why}", False)
    except ValueError as e:
        check(f"schedule refuses: {why}", why in str(e), str(e))
RID = okr.items_from([D("ox", "🥅 O • All done", d(10, 1), d(10, 3), kids=["x1"]),
                      D("x1", "🔑 KR • done", d(10, 1), d(10, 3), parent="ox", status=2)])
try:
    okr.ripple_plan(RID, "ox", d(10, 1), d(10, 6))
    check("extending an O whose every deliverable is done is refused", False)
except ValueError as e:
    check("extending an O whose every deliverable is done is refused", "done" in str(e), str(e))
